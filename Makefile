...
vet: ## Run `go vet` in Docker
	docker build --target vet -f $(CURDIR)/Dockerfile -t toozej/sn2ssg-go:latest . 

test: ## Run `go test` in Docker
	docker build --target test -f $(CURDIR)/Dockerfile -t toozej/sn2ssg-go:latest . 
	@echo -e "\nStatements missing coverage"
	@grep -v -e " 1$$" c.out

build: ## Build Docker image, including running tests
	docker build -f $(CURDIR)/Dockerfile -t toozej/sn2ssg-go:latest .

get-cosign-pub-key: ## Get sn2ssg-go Cosign public key from GitHub
	test -f $(CURDIR)/sn2ssg-go.pub || curl --silent https://raw.githubusercontent.com/toozej/sn2ssg-go/main/sn2ssg-go.pub -O

verify: get-cosign-pub-key ## Verify Docker image with Cosign
	cosign verify --key $(CURDIR)/sn2ssg-go.pub toozej/sn2ssg-go:latest

run: ## Run built Docker image
	docker run --rm --name sn2ssg-go -v $(CURDIR)/config:/config toozej/sn2ssg-go:latest
...
distroless-build: ## Build Docker image using distroless as final base
	docker build -f $(CURDIR)/Dockerfile.distroless -t toozej/sn2ssg-go:distroless . 

distroless-run: ## Run built Docker image using distroless as final base
	docker run --rm --name sn2ssg-go -v $(CURDIR)/config:/config toozej/sn2ssg-go:distroless
...
local-release: local-test docker-login ## Release assets using locally installed golang toolchain and goreleaser
	if test -e $(CURDIR)/sn2ssg-go.key && test -e $(CURDIR)/.env; then \
		export `cat $(CURDIR)/.env | xargs` && goreleaser release --rm-dist; \
	else \
		echo "no cosign private key found at $(CURDIR)/sn2ssg-go.key. Cannot release."; \
	fi

local-sign: local-test ## Sign locally installed golang toolchain and cosign
	if test -e $(CURDIR)/sn2ssg-go.key && test -e $(CURDIR)/.env; then \
		export `cat $(CURDIR)/.env | xargs` && cosign sign-blob --key=$(CURDIR)/sn2ssg-go.key --output-signature=$(CURDIR)/sn2ssg-go.sig $(CURDIR)/out/sn2ssg-go; \
	else \
		echo "no cosign private key found at $(CURDIR)/sn2ssg-go.key. Cannot release."; \
	fi
...
local-verify: get-cosign-pub-key ## Verify locally compiled binary
	...
	cosign verify-blob --key $(CURDIR)/sn2ssg-go.pub --signature $(CURDIR)/sn2ssg-go.sig $(CURDIR)/out/sn2ssg-go

install: local-build local-verify ## Install compiled binary to local machine
	sudo cp $(CURDIR)/out/sn2ssg-go /usr/local/bin/sn2ssg-go
	sudo chmod 0755 /usr/local/bin/sn2ssg-go
...
pre-commit-run: ## Run pre-commit hooks against all files
	...
	go-licenses report github.com/toozej/sn2ssg-go/cmd/sn2ssg-go --ignore `go list std | awk 'NR > 1 { printf(",") } { printf("%s",$$0) } END { print "" }'`
	govulncheck ./...
...
docs-generate:
	docker build -f $(CURDIR)/Dockerfile.docs -t toozej/sn2ssg-go:docs . 
	docker run --rm --name sn2ssg-go-docs -v $(CURDIR):/package -v $(CURDIR)/docs:/docs toozej/sn2ssg-go:docs

docs-serve: ## Serve documentation on http://localhost:9000
	docker run -d --rm --name sn2ssg-go-docs-serve -p 9000:3080 -v $(CURDIR)/docs:/data thomsch98/markserv
	$(OPENER) http://localhost:9000/docs.md
	@echo -e "to stop docs container, run:\n"
	@echo "docker kill sn2ssg-go-docs-serve"

clean: ## Remove any locally compiled binaries
	rm -f $(CURDIR)/out/sn2ssg-go
