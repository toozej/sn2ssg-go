...
func trashSNCLILog(inputLines []string) []string {
	...
	for _, line := range inputLines {
		log.Debug("trashSNCLILog working on line: ", line)
...
func main() {
	...
	log.Debug("inputLines before trashing: ", inputLines)
	inputLines = trashSNCLILog(inputLines)
	log.Debug("inputLines after trashing: ", inputLines)

	notes := splitNotes(inputLines)
	log.Debug("split notes:", notes)
	for _, note := range notes {
		log.Debug("main working on note: ", note)
...
// general TODOs below
// DONE make config work off env vars from .env like ghouls does
...
// TODO functions should operate on the type/struct for note and ssg
...
// TODO enable packages on GH https://github.com/users/toozej/packages/container/sn2ssg-go
